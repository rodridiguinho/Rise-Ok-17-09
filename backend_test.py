...
def test_critical_rui_data_recovery():
    ...
    # Test 4: INVESTIGAÇÃO DETALHADA DA TRANSAÇÃO DO RUI
    print("\n🎯 MISSÃO 2: INVESTIGAÇÃO DETALHADA DA TRANSAÇÃO DO RUI")
    try:
        if 'rui_transactions' in locals() and rui_transactions:
            rui_transaction = rui_transactions[0]  # A transação encontrada
            transaction_id = rui_transaction.get('id')
            
            print(f"\n🔍 ANÁLISE DETALHADA DA TRANSAÇÃO DO RUI:")
            print(f"   🆔 ID: {transaction_id}")
            print(f"   📝 Descrição: {rui_transaction.get('description')}")
            print(f"   💰 Valor: R$ {rui_transaction.get('amount', 0):,.2f}")
            print(f"   📅 Data: {rui_transaction.get('date')}")
            print(f"   🏷️ Tipo: {rui_transaction.get('type')}")
            print(f"   📂 Categoria: {rui_transaction.get('category')}")
            print(f"   👤 Cliente: {rui_transaction.get('client') or 'Não informado'}")
            print(f"   🏢 Fornecedor: {rui_transaction.get('supplier') or 'Não informado'}")
            print(f"   💳 Método Pagamento: {rui_transaction.get('paymentMethod')}")
            print(f"   🔒 Oculta do Controle: {rui_transaction.get('hiddenFromPassengerControl', False)}")
            
            # Verificar se é uma transação gerada automaticamente
            if rui_transaction.get('autoGenerated'):
                print(f"   🤖 Gerada Automaticamente: {rui_transaction.get('autoGenerated')}")
                original_id = rui_transaction.get('originalTransactionId')
                if original_id:
                    print(f"   🔗 ID Transação Original: {original_id}")
                    
                    # Buscar a transação original
                    original_transaction = None
                    for t in all_transactions:
                        if t.get('id') == original_id or str(t.get('_id')) == original_id:
                            original_transaction = t
                            break
                    
                    if original_transaction:
                        print(f"\n📋 TRANSAÇÃO ORIGINAL ENCONTRADA:")
                        print(f"   🆔 ID Original: {original_transaction.get('id')}")
                        print(f"   📝 Descrição Original: {original_transaction.get('description')}")
                        print(f"   💰 Valor Original: R$ {original_transaction.get('amount', 0):,.2f}")
                        print(f"   👤 Cliente Original: {original_transaction.get('client') or 'Não informado'}")
                        print(f"   🔒 Original Oculta: {original_transaction.get('hiddenFromPassengerControl', False)}")
                        
                        # Esta é provavelmente a transação principal do Rui!
                        if original_transaction.get('amount') == 14000.0 or 'rui' in (original_transaction.get('client') or '').lower():
                            print_result(True, "🎯 TRANSAÇÃO PRINCIPAL DO RUI ENCONTRADA", 
                                       f"✅ A transação original (ID: {original_transaction.get('id')}) pode ser a venda principal do Rui")
                    else:
                        print_result(False, "❌ TRANSAÇÃO ORIGINAL NÃO ENCONTRADA", 
                                   f"❌ Não foi possível encontrar a transação original com ID: {original_id}")
            
            print_result(True, "🔍 INVESTIGAÇÃO DETALHADA CONCLUÍDA", 
                       f"✅ Transação do Rui analisada em detalhes")
        
        # Buscar por valor próximo a R$ 14.000,00 (pode ter variações)
        print(f"\n🔍 BUSCA POR VALORES PRÓXIMOS A R$ 14.000,00:")
        value_range_transactions = []
        for t in all_transactions:
            amount = t.get('amount', 0)
            if 13000 <= amount <= 15000:  # Range de R$ 13.000 a R$ 15.000
                value_range_transactions.append(t)
        
        if value_range_transactions:
            print_result(True, "💰 TRANSAÇÕES NA FAIXA R$ 13.000-15.000", 
                       f"✅ Encontradas {len(value_range_transactions)} transação(ões) na faixa de valor")
            for transaction in value_range_transactions:
            ...
            print_result(False, "💰 TRANSAÇÕES NA FAIXA R$ 13.000-15.000", 
                       "❌ Nenhuma transação encontrada na faixa de R$ 13.000-15.000")
        
        # Buscar por "Rui Manuel" especificamente
        print(f"\n🔍 BUSCA ESPECÍFICA POR 'RUI MANUEL':")
        rui_manuel_transactions = []
        for t in all_transactions:
            client = (t.get('client') or '').lower()
            description = (t.get('description') or '').lower()
            if 'rui manuel' in client or 'rui manuel' in description:
                rui_manuel_transactions.append(t)
        
        if rui_manuel_transactions:
            print_result(True, "👤 TRANSAÇÕES COM 'RUI MANUEL'", 
                       f"✅ Encontradas {len(rui_manuel_transactions)} transação(ões) com 'Rui Manuel'")
            for transaction in rui_manuel_transactions:
                print(f"   🆔 ID: {transaction.get('id')}, Cliente: {transaction.get('client')}, Valor: R$ {transaction.get('amount', 0):,.2f}")
        else:
...
if __name__ == "__main__":
    print("🚨 RECUPERAÇÃO URGENTE - DADOS DO RUI PERDIDOS")
    print(f"🔗 Backend URL: {BASE_URL}")
    print(f"🔗 API URL: {API_URL}")
    print(f"👤 Credenciais: {VALID_EMAIL}")
    print("🎯 MISSÃO: Encontrar e restaurar dados do cliente Rui")
    
    # Run critical Rui data recovery mission
    test_critical_rui_data_recovery()
    
    print("\n" + "="*80)
    print("🏁 MISSÃO DE RECUPERAÇÃO CONCLUÍDA")
    print("="*80)
